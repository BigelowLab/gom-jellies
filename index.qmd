---
title: "gom-jellies"
---

```{r setup, echo = FALSE, include = FALSE}
source("setup.R")
coast = read_coast()
coast_bbox = st_bbox(coast)

all = read_obs() %>%
  filter(source == "record") %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)
obs = read_obs() %>%
  filter(source == "record", type %in% c("lionsmane", "moon", "whitecross", "comb")) %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326) %>%
  mutate(year = year(date),
         month = factor(month(date, label = TRUE, abbr = TRUE),
                        levels = month.abb),
         type = as.factor(type))

# df = read.csv("jellyfish-prod - prod.csv") %>% 
#   select(Source, Year, Month, Day, Time, Type, Lat, Lon, Count, Size) %>% 
#   drop_na() %>%
#   mutate(Date = as.Date(sprintf("%04d-%02d-%02d", Year, Month, Day))) %>% 
#   st_as_sf(coords = c("Lon", "Lat"), crs = 4326) %>%
#   select(-Year, -Month, -Day) %>%
#   rename(
#     date = Date,
#     source = Source,
#     type = Type
#   )
# 
# all = df %>% 
#   filter(source == "record")
# 
# obs = df %>% 
#   filter(source == "record", type %in% c("lionsmane", "moon", "whitecross", "comb")) %>% 
#     mutate(year = year(date),
#          month = factor(month(date, label = TRUE, abbr = TRUE), 
#                         levels = month.abb),
#          type = as.factor(type))
```

For the [gom-jellies](https://github.com/BigelowLab/gom-jellies) project, observations come from community scientist reports. The following species have been selected for analysis:

  + *Cyanea capillata* (Lion's Mane)
  + *Aurelia aurita* (Moon)
  + *Staurostoma mertensii* (White Cross)
  + *Ctenophores* (Comb)

# All Observations
<<<<<<< HEAD
=======
Distribution of the `r length(all$type)` reports for all species.
>>>>>>> 6d61a7a1680c9f53df050073fa95d0174798852f
```{r, echo = FALSE}
# Distribution of the `r length(all$type)` reports for all species.
ggplot() +
  geom_sf(data = coast, color = "black") +
  geom_sf(data = all, color = "steelblue", size = 1.5) +
  coord_sf(
    xlim = c(coast_bbox["xmin"], coast_bbox["xmax"]),
    ylim = c(coast_bbox["ymin"], coast_bbox["ymax"]),
    expand = FALSE
  ) +
  labs(title = paste("Distribution of", nrow(all), "Observations")) + 
  theme_minimal() +
  theme(axis.title = element_blank())
```

# Filtered Set 
Here we show the distribution of the `r length(obs$type)` lion's mane, moon, white cross, and comb jellies reports.
```{r, echo = FALSE}
ggplot() +
  geom_sf(data = coast, color = "black") +
  geom_sf(data = obs, color = "steelblue", size = 1.5) +
  coord_sf(
    xlim = c(coast_bbox["xmin"], coast_bbox["xmax"]),
    ylim = c(coast_bbox["ymin"], coast_bbox["ymax"]),
    expand = FALSE
  ) +
  theme_minimal() +
  theme(axis.title = element_blank())

ggplot(obs, aes(x = factor(year))) +
  geom_bar(fill = "steelblue") +
  theme_minimal() +
  labs(
    title = "Number of Observations per Year",
    x = "Year",
    y = "Count"
  )

ggplot(obs, aes(x = factor(month))) +
  geom_bar(fill = "steelblue") +
  theme_minimal() +
  labs(
    title = "Number of Observations per Month",
    x = "Month",
    y = "Count"
  )
```

# By Species
```{r, echo = FALSE}
ggplot() +
  geom_sf(data = coast, color = "black") +
  geom_sf(data = obs, aes(color = type), size = 1.5) +
  coord_sf(
    xlim = c(coast_bbox["xmin"], coast_bbox["xmax"]),
    ylim = c(coast_bbox["ymin"], coast_bbox["ymax"]),
    expand = FALSE
  ) +
  theme_minimal() +
  theme(axis.title = element_blank())

ggplot(obs, aes(x = factor(year), fill = type)) +
  geom_bar() +
  theme_minimal() +
  labs(
    title = "Number of Observations per Year",
    x = "Year",
    y = "Count"
  )

ggplot(obs, aes(x = factor(month), fill = type)) +
  geom_bar() +
  theme_minimal() +
  labs(
    title = "Number of Observations per Month",
    x = "Month",
    y = "Count"
  )

```
